...
read.csv('Germany Incidence.csv') %>%
  filter(month>=2014.75) -> inc
read.csv('Germany covar YearlyDistance.csv') %>%
...
inc %>%
  ...
  geom_line() +
  geom_point() +
  labs(x='Time(monthly)',y=expression(paste('Incidence'%*%10^4)),title='Germany') +
  theme_bw() -> f.inc
...
cov %>%
  ...
  labs(x='Time(monthly)',y='H3N2 dist',title='Germany') +
  theme_bw() -> f.Et.H3N
...
cov %>%
  ...
  labs(x='Time(monthly)',y='H1N1 dist',title='Germany') +
  theme_bw() -> f.Et.H1N
...
tsim %>% 
  mif2(params=guess[1,],
       ...
       rw.sd=rw.sd(sigma=0.02,
                   ...
                   c1=0.02,c2=0,omega=0.02,xi=0.02,P1=0,P2=0,
         ...
         .errorhandling="remove", .inorder=FALSE) %dopar% {
           mf1 %>%  
            mif2(params=start,rw.sd=rw.sd(sigma=0.01,
                                          ...
                                          c1=0.01,c2=0,omega=0.01,xi=0.01,P1=0,P2=0,
...
write.csv(estimates,'GermanyEstimates.csv', row.names = F)
...
write.csv(sim1_H1N_summary,'Germany sim1_H1N_summary.csv', row.names = F)
write.csv(sim1_H3N_summary,'Germany sim1_H3N_summary.csv', row.names = F)
...
#write.csv(Et,'GermanyEt.csv', row.names = F)
