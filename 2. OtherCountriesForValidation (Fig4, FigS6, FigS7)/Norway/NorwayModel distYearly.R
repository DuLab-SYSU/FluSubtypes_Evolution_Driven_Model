...
read.csv('Norway Incidence.csv') %>%
  filter(month>=2014.75) -> inc
read.csv('Norway covar YearlyDistance.csv') %>%
...
inc %>%
  ...
  geom_line() +
  geom_point() +
  labs(x='Time(monthly)',y=expression(paste('Incidence'%*%10^4)),title='Norway') +
  theme_bw() -> f.inc
...
cov %>%
  ...
  labs(x='Time(monthly)',y='H3N2 dist',title='Norway') +
  theme_bw() -> f.Et.H3N
...
cov %>%
  ...
  labs(x='Time(monthly)',y='H1N1 dist',title='Norway') +
  theme_bw() -> f.Et.H1N
...
write.csv(estimates,'NorwayEstimates.csv', row.names = F)
...
write.csv(sim1_H1N_summary,'Norway sim1_H1N_summary.csv', row.names = F)
write.csv(sim1_H3N_summary,'Norway sim1_H3N_summary.csv', row.names = F)
...
figure_H1N <- ggplot(sim1_H1N_summary,aes(x = month,y = mean,ymin = lower,ymax = upper))+
  ...
  labs(y=expression(paste('H1N1 Incidence'%*%10^4)),x="Time(monthly)")+
...
write.csv(Et,'NorwayEt.csv', row.names = F)
